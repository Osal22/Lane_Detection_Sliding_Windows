...
while success:
    ...
    try:
        ...
        # cv2.waitKey(1)
        restored_frame = cv2.warpPerspective(msk, inverse_matrix, (640,480))
        alpha = restored_frame / 255.0
        # Create a mask by duplicating the alpha channel to all three RGB channels
        mask = cv2.merge((alpha, alpha, alpha))
        # Apply the mask to the RGB image
        # result = cv2.addWeighted(frame, 1 - alpha, mask, alpha, 0)
        result=customAddWeighted(mask,alpha,frame,(1-alpha),0)        
